...
import { Document, Page, PDFViewer, Text, Font } from "@react-pdf/renderer";
...
const MyDoc = ({
  ...
  return (
    <Document
      title={`${projectData.project_id}-${projectData.funding_agency}_SA`}
    >
      <Page size="A4" style={{ padding: 20, fontFamily: "Noto Sans" }}>
        ...
        <Text style={{ fontSize: 12, marginBottom: 20 }}>
          ...
          {projectData.copis.length
            ? projectData.copis.map((pi) => pi.name).join(" ,")
            : "None Specified"}
        </Text>
        ...
        <Table data={rows}>
          <TableHeader textAlign="center" fontSize={10}>
            ...
            <TableCell>
              <Text style={{ fontWeight: 700 }}>Initial Amount</Text>
            </TableCell>
            <TableCell>
              <Text style={{ fontWeight: 700 }}>Carry Forward</Text>
            </TableCell>
            <TableCell>
              <Text style={{ fontWeight: 700 }}>Total Current</Text>
            </TableCell>
            <TableCell>
          ...
          <TableBody>
            <DataTableCell
              getContent={(r: Row) => (
                <Text
                  {...(r.head === "Total" && { style: { fontWeight: 700 } })}
                >
                  {r.head}
                </Text>
              )}
            ...
            <DataTableCell
              getContent={(r: Row) => (
                <Text
                  {...(r.head === "Total" && { style: { fontWeight: 700 } })}
                >
                  {formatCurrency(r.initialAmount)}
                </Text>
              )}
            />
            <DataTableCell
...
export const ProjectSAViewer = () => {
  ...
  useEffect(() => {
    fetch(
      ...
      }/api/project/${id}/total-expenses?projectData=true`,
      {
        credentials: "include",
      }
    )
