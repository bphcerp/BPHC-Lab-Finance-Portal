...
const ExpensesPage: React.FC = () => {
  ...
  const [isFileReimbursementModalOpen, setIsFileReimbursementModalOpen] =
    useState(false);
  ...
  const columns = [
    ...
    columnHelper.accessor(
      (row) => (row.settled ? row.settled.type : "Not Settled"),
      {
        header: "Settled",
        cell: (info) => (
          <span
  ...
  const handleSettleExpenses = async (
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/expense/settle`,
        {
          method: "PATCH",
          credentials: "include",
          headers: {
  ...
  const handleDeleteExpense = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/expense/${expenseToDelete._id}`,
        {
          credentials: "include",
          method: "DELETE",
        }
  ...
  return expenses ? (
    <div className="flex flex-col">
      <AddExpenseModal
        ...
        {...(selectedExpense
          ? { edit: "Normal", editData: selectedExpense }
          : {})}
      ...
      <SettleExpenseModal
        ...
        selectedExpenses={expenses.filter(
          (expense) =>
            selectedExpenses.has(expense._id) && expense.paidBy !== undefined
        )}
      ...
      <FileReimbursementModal
        ...
        selectedExpenses={expenses.filter((expense) =>
          selectedExpenses.has(expense._id)
        )}
            ...
            <div className="flex space-x-2">
              ...
              {selectedExpenses.size > 0 ? (
                <div className="flex space-x-2">
                  <Button
                    ...
                    onClick={() => {
                      const eligibleExpenses = expenses
                        .filter((expense) => selectedExpenses.has(expense._id))
                        .filter(
                          (expense) =>
                            expense.paidBy &&
                            !expense.settled &&
                  ...
                  >
                    {"Settle Expense" + (selectedExpenses.size > 1 ? "s" : "")}
                  </Button>
                  <Button
                    color="gray"
                    size="md"
                    ...
                    onClick={() => {
                      const eligibleExpenses = expenses
                        .filter((expense) => selectedExpenses.has(expense._id))
                        .filter((expense) => !expense.reimbursedID);
                      if (eligibleExpenses.length === 0) {
        ...
        {...(user?.role === "Admin"
          ? {
              setSelected: (selectedExpenses: Array<Expense>) =>
                setSelectedExpenses(
                  new Set(selectedExpenses.map((expense) => expense._id))
                ),
            }
          : {})}
        initialState={{
