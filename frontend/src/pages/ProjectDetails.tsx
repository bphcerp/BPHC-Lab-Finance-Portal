...
const ProjectDetails = () => {
  ...
  const [instituteExpenses, setInstituteExpenses] = useState<
    Array<InstituteExpense>
  >([]);
  ...
  const getCarryTotal = () => {
    return currentYear
      ? Object.values(projectData!.carry_forward).reduce(
          (sum, arr) => sum + (arr[currentYear - 1] || 0),
          0
        )
      : 0;
  };
  ...
  const fetchReimbursements = async ({
    ...
    try {
      ...
      setLabel(
        ` ${head ?? ""}${all ? projectData?.funding_agency : ""}${
          index !== undefined
            ? ` ${
                projectData?.project_type === "invoice" ? "Installment" : "Year"
              } ${index + 1}`
            : ""
        }`
      );
  ...
  const handleExport = async () => {
    try {
      ...
      link.download = `${projectData?.project_id}-${
        projectData?.funding_agency
      }${head ? ` ${head}` : ""}${
        index !== undefined
          ? projectData?.project_type === "invoice"
            ? " Installment "
            : " Year "
          : ""
      }${index !== undefined ? index + 1 : ""} Expense Data.xlsx`;
      link.click();
    } catch (error) {
  ...
  return (
    <>
      {projectData && (
        <div className="relative flex flex-col space-y-6 w-full mx-4">
          ...
          <OverrideConfirmation
            ...
            reset={resetOverride}
            max={
              projectData.project_type === "yearly"
                ? calculateNumberOfYears(
                    new Date(projectData.start_date!),
                    new Date(projectData.end_date!)
                  )
                : projectData.installments!.length
            }
            onClose={() => {
              setIsOverrideModalOpen(false);
              setResetOverride(false);
          ...
          <div className="grid grid-cols-4 gap-4">
            <div className="col-span-3">
              <div className="flex justify-between mb-2">
                <div className="flex space-x-2 items-center w-fit">
                  <span className="text-2xl font-semibold text-gray-70">
                    Project Data
                  </span>
                  <span className="">
                    (Click on the amount to view the reimbursements)
                  </span>
                </div>
                <button
              ...
              <div className="overflow-x-auto scroll-smooth">
                {Object.keys(projectData.project_heads).length ? (
                  <table className="bg-white min-w-full shadow-md rounded-lg">
                    <thead className="bg-gray-200">
                      <tr className="bg-inherit">
                        ...
                        {Array.from({
                          ...
                          <th
                            key={i}
                            {...(!isProjectOver && currentYear === i
                              ? { ref: targetColumnRef }
                              : {})}
                            className={`py-3 px-6 text-center bg-inherit text-gray-600 ${
                              !isProjectOver && currentYear === i
                                ? "text-red-600"
                                : "   "
                            }`}
                          >
                            <div className="flex flex-col space-y-2">
                              <button
                                className="text-blue-600 text-lg hover:underline"
                                onClick={() =>
                                  fetchReimbursements({ index: i, all: true })
                                }
                              >
                                {projectData.project_type === "invoice"
                                  ? "Installment"
                              ...
                              {projectData.project_type === "invoice" ? (
                                <span>
                                  {formatDate(
                                    projectData.installments![i].start_date
                                  )}{" "}
                                  -{" "}
                                  {formatDate(
                                    projectData.installments![i].end_date
                                  )}
                                </span>
                              ) : (
                                <></>
                            ...
                            {allocations.map((amount, i) => (
                              ...
                              >
                                <button
                                  ...
                                  onClick={() =>
                                    fetchReimbursements({ head, index: i })
                                  }
                                >
                  ...
                  <div className="flex items-center justify-center text-2xl bg-gray-200 w-full h-56 rounded-lg ">
            ...
            <div className="flex flex-col space-y-4">
              ...
              <div className="piDetails flex flex-col space-y-3 p-4 font-semibold w-full bg-gray-100 rounded-md shadow-md">
                ...
                <div className="flex items-center">
                  <span className="inline-block w-20 text-gray-700">
                    Co-PIs:
                  </span>
                  <span className="text-gray-900">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-2">
              <span className="text-2xl font-semibold text-gray-700">
                Current ({" "}
                {projectData.project_type === "yearly" ? "Year" : "Installment"}{" "}
                {currentYear + 1} ) Expense Sheet
              </span>
              {user?.role === "Admin" &&
                !isProjectOver &&
                ((
                  projectData.project_type === "invoice"
                    ? currentYear + 1 === projectData.installments!.length
                    : currentYear + 1 ===
                      calculateNumberOfYears(
                        new Date(projectData.start_date!),
                        new Date(projectData.end_date!)
            ...
            <Link
              target="_blank"
              rel="noopener noreferrer"
              to={`accounts/pdfviewer?index=${currentYear}`}
            >
              <Button color="success">View SA</Button>
            </Link>
          </div>
                ...
                <tbody>
                        ...
                        <tr key={index} className="border-t">
                          ...
                          {currentYear ? (
                            <>
                              <td className="py-3 px-6 text-center text-gray-600">
                                {formatCurrency(
                                  projectData.carry_forward[head][
                                    currentYear - 1
                                  ]
                                )}
                              </td>
                          ...
                          <td className="py-3 px-6 text-center text-gray-600">
                            ...
                            >
                              {expenseData
                                ? formatCurrency(expenseData[head] ?? 0)
                                : "Loading"}
                            </button>
                          </td>
                          <td className="py-3 px-6 text-center text-gray-600">
                  ...
                  <tr className="border-t bg-gray-100 font-semibold">
                    ...
                    {currentYear ? (
                      <>
                        <td className="py-3 px-6 text-gray-800 text-center">
                          {formatCurrency(getCarryTotal())}
                        </td>
                        <td className="py-3 px-6 text-gray-800 text-center">
                          {formatCurrency(getProjectTotal() + getCarryTotal())}
                        </td>
                      </>
                    ) : (
                      <></>
      ...
      {projectData && expenseData && isCarryModalOpen && (
        <CarryConfirmationModal
      ...
      {id && projectData && expenseData && isCarryDetailsModalOpen && (
        <CarryDetailsModal
          projectId={id}
          projectHeads={projectData.project_heads}
