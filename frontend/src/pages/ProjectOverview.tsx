...
const ProjectList: FunctionComponent = () => {
  ...
  const fetchProjectData = () => {
    fetch(
      `${import.meta.env.VITE_BACKEND_URL}/api/project?past=true&balance=true`,
      {
        credentials: "include",
      }
    )
      .then(async (res) => {
        ...
        setUniqueHeads(getUniqueProjectHeads(processedData));
      })
      .catch((e) => {
  ...
  const fetchReimbursements = async ({
    ...
    try {
      ...
      setLabel(
        ` ${project.funding_agency} ${head ?? ""}${
          index !== undefined
            ? ` ${
                project.project_type === "invoice" ? "Installment" : "Year"
              } ${index + 1}`
            : ""
        }`
      );
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/reimburse/${
          project._id
        }/?exportData=true&head=${head}&index=${index}`,
        { credentials: "include" }
      );
      ...
      link.download = `${project.funding_agency}${head ? ` ${head}` : ""}${
        index !== undefined
          ? project.project_type === "invoice"
            ? " Installment "
            : " Year "
          : ""
  ...
  const columns = [
    ...
    columnHelper.accessor("project_title", {
      ...
      cell: (info) => (
        <Link
          ...
          to={`/project/${info.row.original._id}`}
        >
          {info.getValue()}
        </Link>
      ),
    ...
    columnHelper.accessor(
      (row) =>
        row.project_type.charAt(0).toUpperCase() + row.project_type.slice(1),
      {
        header: "Project Type",
        cell: ({ getValue }) => (
          <span
            className={
              getValue() === "Yearly" ? "text-violet-500" : "text-yellow-400"
            }
          >
            {getValue()}
          </span>
    ...
    columnHelper.accessor(
      (row) => (getCurrentIndex(row) >= 0 ? "Ongoing" : "Ended"),
      {
        header: "Status",
        cell: (info) => {
          return (
    ...
    columnHelper.accessor("total_amount", {
      ...
      cell: (info) =>
        getCurrentIndex(info.row.original) !== -1 ? (
          <button
            className="text-blue-600 hover:underline"
            onClick={() => fetchReimbursements({ project: info.row.original })}
          >
            {(info.row.original.total_amount ?? 0).toLocaleString("en-IN", {
              style: "currency",
              currency: "INR",
            })}
          </button>
        ...
        columnHelper.accessor((row) => `${row.funding_agency}_${head}`, {
          ...
          cell: (info) =>
            getCurrentIndex(info.row.original) === -1 ? (
              ...
              <button
                className={
                  info.row.original.project_heads[head] !== undefined
                    ? `text-blue-600 hover:underline`
                    : ""
                }
                onClick={() =>
                  fetchReimbursements({ project: info.row.original, head })
                }
              >
                {(info.row.original.project_heads[head] ?? 0).toLocaleString(
                  "en-IN",
                  {
                    style: "currency",
                    currency: "INR",
