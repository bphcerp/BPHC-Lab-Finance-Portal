...
const AccountPage: FunctionComponent<AccountPageProps> = ({ type }) => {
  ...
  const [selectedAccountEntries, setSelectedAccountEntries] =
    useState<Account[]>();
  const { user } = useUser();
    ...
    () => [
      columnHelper.accessor((row) => (row.transferable ? "Yes" : "No"), {
        ...
        meta: {
          ...
          getSum: true,
          sumFormatter: (sum: number) => (
            <span className={sum < 0 ? "text-green-400" : "text-red-600"}>
              {sum.toLocaleString("en-IN", {
                style: "currency",
                currency: "INR",
              })}
      ...
      columnHelper.accessor(
        (row) =>
          row.transfer
            ? row.transfer.amount < row.transferable
              ? "Partial"
              : "Full"
            : "No",
        {
          ...
          cell: ({ row }) => (
            <span
              className={`inline-block px-3 py-1 w-full text-center rounded-full text-xs font-semibold ${
                !row.original.transfer
                  ? "bg-red-100 text-red-800"
                  : row.original.transfer.amount >= row.original.transferable
            ...
            >
              {row.original.transferable
                ? !row.original.transfer
                  ? "No Transfer"
                  : formatCurrency(row.original.transfer.amount)
                : "NA"}
            </span>
          ),
          meta: {
  ...
  const handleTransferDelete = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/account/${entryToDelete?._id}`,
  ...
  useEffect(() => {
    fetchAccountData().then(() =>
      setColumns(
        type === "Current" ? [...baseColumns, ...currentColumns] : baseColumns
      )
    );
