...
const EditProjectModal: React.FC<EditProjectModalProps> = ({
  ...
  const handleAddPi = () => {
    const selectedPiMember = faculties.find(
      (faculty) => faculty._id === selectedPi
    );
    if (selectedPiMember) {
  ...
  const handleAddCopi = () => {
    const selectedCoPiMember = faculties.find(
      (faculty) => faculty._id === selectedCopi
    );
  ...
  const handleAddHead = () => {
    ...
    setValue(
      `project_heads.${newHeadName}`,
      new Array(
        project?.project_type === "yearly"
          ? calculateNumberOfYears(project!.start_date, project!.end_date)
          : project!.installments!.length
      ).fill(0)
    ); // Initialize with 5 installments of 0
  ...
  const handleHeadDelete = async (headName: string) => {
    ...
    if (!project?.project_heads[headName]) {
      ...
      setValue(
        `negative_heads`,
        watch("negative_heads")?.filter(
          (negativeHead) => negativeHead !== headName
        )
      );
      return;
    }
      ...
      <Modal.Body>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="border-t pt-4 space-y-4">
            ...
            {Object.entries(watch("project_heads") || {}).map(
              ([headName, amounts], index) => (
                      ...
                      <Checkbox
                        ...
                        onChange={() => {
                          if (watch("negative_heads")?.includes(headName)) {
                            ...
                            setValue("negative_heads", [
                              ...watch("negative_heads"),
                              headName,
                            ]);
                          }
                        }}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {amounts.map((amount, i) => (
                      <TextInput
                        ...
                        type="number"
                        defaultValue={amount}
                        {...register(`project_heads.${headName}.${i}`, {
                          valueAsNumber: true,
                        })}
          ...
          <div>
            ...
            <TextInput
              id="totalAmount"
              type="number"
              value={Object.values(watch("project_heads")).reduce(
                (sum, alloc) => {
