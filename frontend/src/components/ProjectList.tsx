...
const ProjectList: FunctionComponent = () => {
  ...
  const fetchProjectData = () => {
    fetch(`${import.meta.env.VITE_BACKEND_URL}/api/project/?past=true`, {
      credentials: "include",
    })
      .then(async (res) => {
        if (!res.ok) {
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      setProjectData(
        projectData.filter((project) => project._id !== projectToDelete._id)
      );
  ...
  return projectData ? (
    <div className="px-4">
      <DeleteConfirmationModal
        ...
        item={
          projectToDelete
            ? `${projectToDelete.funding_agency}-${projectToDelete.project_title}`
            : ""
        }
      />
      <DescriptionModal
