...
function TableCustom<T>({
  ...
  // Normalize & enrich columns: add filterFn for dropdowns and ensure stable id
        ...
        if (!col.id) {
          const generatedId =
            typeof col.header === "string"
              ? col.header.replace(/\s+/g, "_").toLowerCase()
              : col.accessorKey || `col_${idx}`;
          col = { ...col, id: generatedId };
        }
        return col;
      }),
    [columns]
  ...
  const focusRow = (targetId: string) => {
    ...
    const rowIndex = tableRows.findIndex(
      (row) => (row.original as { _id: string })._id === targetId
    );
    ...
    console.log(
      ...
      "hehe",
      targetId,
      table.getPrePaginationRowModel().rows[0].original
    );
  ...
  useEffect(() => {
    if (showRowId && table.getPrePaginationRowModel().rows.length)
      focusRow(showRowId);
    ...
    table.getAllColumns().forEach((column) => {
      if (column.columnDef.meta?.getSum) {
        sums[column.id] = table.getRowModel().rows.reduce((sum, row) => {
          const value = column.columnDef.meta?.filterType
            ? row.original[column.id.toLowerCase() as keyof typeof row.original]
          ...
          {table.getHeaderGroups().map((headerGroup) => (
            <Table.Head className="bg-gray-200" key={headerGroup.id}>
              ...
              {headerGroup.headers.map((header) => (
                <Table.HeadCell
                  className={`bg-inherit whitespace-nowrap py-2.5 ${
                ...
                >
                  {header.isPlaceholder ? null : (
                    <>
                      ...
                      >
                        {header.column.columnDef.meta?.truncateLength &&
                        header.column.columnDef.meta?.truncateLength <
                          ...
                          <div className="relative group">
                            ...
                            <span className="absolute left-0 mt-5 opacity-0 transition-opacity duration-700 group-hover:opacity-100 bg-gray-300 text-gray-700 text-xs p-2 rounded shadow-lg">
                          ...
                          {header.column.columnDef.meta ? (
                            ...
                            "dropdown" ? (
                              ...
                              >
                                ...
                                {getSortedUniqueValues(header.column).map(
                                  (value) => (
          ...
          <Table.Body>
            {table.getRowModel().rows.map((row, index) => (
              <Table.Row
                ...
                key={row.id}
                className={`${index % 2 ? "bg-gray-100" : "bg-white"} border ${
              ...
              >
                ...
                {row.getVisibleCells().map((cell) => (
                  <Table.Cell
                    className="text-gray-700 px-6  py-2.5"
                    key={cell.id}
                  >
              ...
              <Table.Row className="bg-gray-200 text-black font-bold text-lg">
                ...
                {table.getVisibleLeafColumns().map((column) => (
                  <Table.Cell key={column.id} className="px-0 py-2.5">
                    {column.columnDef.meta?.getSum
                      ? column.columnDef.meta?.sumFormatter?.(
                          columnSums[column.id]
                        ) ?? columnSums[column.id].toLocaleString()
                      : null}
                  </Table.Cell>
                ))}
              </Table.Row>
